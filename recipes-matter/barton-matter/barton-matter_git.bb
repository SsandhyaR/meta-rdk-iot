...
SRCREV = "4564cd2e0a0c7059bb99719cfc3de50cefac5d10"
...
do_configure:prepend() {
    ...
    if [ ! -L "${S}/third_party/barton/build_overrides" ]; then
    ...
    if [ ! -L "${S}/third_party/barton/src/app/zap-templates/zcl" ]; then
    ...
    if [ ! -L "${S}/third_party/barton/third_party/connectedhomeip" ]; then
    ...
    # Since /tmp/zap could point to another package's sysroot, we have to remove it and
    # remake it for this sysroot. This allows support for multiple versions of barton-matter
    # to be buildable.
    if [ "$(readlink /tmp/zap)" != "${STAGING_DIR_NATIVE}/zap" ]; then
        rm "/tmp/zap" 2>/dev/null || true
